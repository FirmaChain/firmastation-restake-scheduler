...
export class RestakeTaskService {
  ...
  async startScheduler(
    callback: (message: string, notiMessage?: string) => void,
  ) {
    try {
      ...
      if (cronJobs.has(this.jobName)) {
        ...
        const cronJob = new CronJob(CronExpression.EVERY_4_HOURS, async () => {
          ...
          const { resultMsg, failedMsg } = this.makeMessageByRestakeResult(
            restakeResult.data,
            restakeResult.round,
          );
  ...
  getRestakeInfo(callback: (message: string) => void) {
    const minimumRewards = this.configService.get<number>(
...
Retry Count : ${retryCount}`;
  ...
  private async restakeSchedulerTask() {
    try {
      ...
      const dbWriteResult = await this.mongoDbService.saveDB(
        restakeResult,
        scheduleDate,
      );
  ...
  private makeMessageByRestakeResult(
